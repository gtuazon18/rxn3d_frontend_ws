...
export function AddCasePanModal({ isOpen, onClose, editCasePan, onEditDone }: AddCasePanModalProps) {
  ...
  // Generate next available case pan code
  const generateNextCasePanCode = useCallback((): string => {
    // Extract all numeric parts from existing codes that start with "CP"
    const existingCodes = casePans
      .map((cp) => cp.code)
      .filter((code) => code && code.toUpperCase().startsWith("CP"))
      .map((code) => {
        // Extract numeric part after "CP"
        const match = code.toUpperCase().match(/^CP(\d+)$/)
  ...
  const resetForm = () => {
    // Generate auto code for new case pan when resetting
    const autoGeneratedCode = generateNextCasePanCode()
    setFormData({
      name: "",
  ...
  // Populate form when editing (fetch detail if editCasePan is present)
  useEffect(() => {
    let ignore = false
    const fetchDetail = async () => {
      if (editCasePan && editCasePan.id) {
        ...
        // Generate auto code for new case pan
        const autoGeneratedCode = generateNextCasePanCode()
        setFormData({
          name: "",
          code: autoGeneratedCode,
  ...
  }, [editCasePan, isOpen, generateNextCasePanCode])
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editCasePan && isCustomNoState) {
        // Only allow updating quantity and color_code
        if (isLabAdmin) {
  ...
  return (
    <>
                ...
                <div className="px-6 py-5 border-b">
                  ...
                  {casePanDetails && (
                    <div className="space-y-5">
                      ...
                      <Input
                        placeholder={t("csePanModal.casePanCode", "Case Pan Code (e.g., CP0001) *")}
                        className="h-11"
                        value={formData.code}
      ...
      <LoadingOverlay
        ...
        title={editCasePan ? t("csePanModal.updating", "Updating...") : t("csePanModal.creating", "Creating...")}
        message={editCasePan 
