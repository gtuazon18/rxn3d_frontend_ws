...
interface AddCategoryModalProps {
  isOpen: boolean
  onClose: () => void
  editId?: number // Add this prop to indicate edit mode and which subcategory to edit
...
const NO_CASE_PANS_VALUE = "__NO_CASE_PANS__"
...
export function AddCategoryModal({ isOpen, onClose, editId, isEdit, isSubCategoryEdit, disableAllFields, isCopying = false, copyingCategory = null }: AddCategoryModalProps) {
  const {
    createCategory,
    createSubCategory,
    updateCategory, // <-- import updateCategory from context
  ...
  const [detailLoadedId, setDetailLoadedId] = useState<number | null>(null) // Track loaded detail
  ...
  useEffect(() => {
    if (isEdit && isSubCategoryEdit && detailLoadedId) {
      // Image will be loaded from the detail data in the main useEffect
      // This effect is just for cleanup when not editing
    } else {
      setImageBase64(null)
  ...
  useEffect(() => {
    const hasFormChanges =
      ...
      isSubCategory !== (initialFormData.parent_id !== null) ||
      (isSubCategory && formData.parent_id !== initialFormData.parent_id) ||
      (!isSubCategory && formData.parent_id !== null)
    setHasChanges(hasFormChanges)
  }, [formData, initialFormData, isSubCategory])
  ...
  const handleSubCategoryToggle = (checked: boolean) => {
  ...
  const handleSaveCategory = async () => {
    ...
    let zodResult
    if (isSubCategory) {
      zodResult = categorySchema.extend({
        parent_id: z.number({ required_error: "Parent category is required for sub-category." }),
      }).safeParse({ ...formData, parent_id: formData.parent_id })
    } else {
      zodResult = categorySchema.safeParse(formData)
    }
    ...
    const payload = {
      ...
      category_id: isSubCategory ? formData.parent_id : null,
    ...
    try {
      ...
      if (err && err.errors && typeof err.errors === "object") {
        ...
        // Show the first error as the main error message (for accessibility/visibility)
  ...
  const isFormValid =
    ...
    (!isSubCategory || (isSubCategory && formData.parent_id !== null))

  const currentParentDropdownCategories = Array.isArray(parentDropdownCategories?.data) ? parentDropdownCategories.data : []
  const dropdownParentOptions = currentParentDropdownCategories.filter(
    (cat: ProductCategory) => cat.id !== formData.parent_id,
  )
      ...
      <Dialog open={isOpen} onOpenChange={handleClose}>
        <DialogContent className="p-0 gap-0 sm:max-w-[700px] lg:max-w-[800px] xl:max-w-[900px] overflow-hidden bg-white rounded-md">
          ...
          <div className="overflow-y-auto max-h-[calc(80vh-120px)] p-6 space-y-6">
            {isEdit && isSubCategoryEdit && isDetailLoading ? (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="font-medium">
                      {t("categoryModal.details", "Category/Sub Category Details")}
                    </span>
                    ...
                    <div className="flex flex-col lg:flex-row gap-8">
                      ...
                      <div className="flex-1 space-y-6">
                        ...
                        <div className="space-y-4">
                          ...
                          <Input
                            placeholder={t("categoryModal.namePlaceholder", "Category/Sub Category Name *")}
                          ...
                          <div className="space-y-4">
                            <div className="flex items-center space-x-3 p-4 bg-gray-50 rounded-lg">
                              <Checkbox
                                ...
                                disabled={effectiveDisableFields}
                              />
                              <label htmlFor="is-sub-category" className="text-sm font-medium text-gray-700">
                                {t("categoryModal.nestUnder", "Create as Sub Category")}
                              </label>
                            </div>
                            ...
                            {isSubCategory && (
                              <div className="space-y-2">
                                <label className="text-sm font-medium text-gray-700">
                                  {t("categoryModal.nestUnder", "Parent Category")} <span className="text-red-500">*</span>
                                </label>
                                <Select
                                  ...
                                  disabled={!isSubCategory || effectiveDisableFields}
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                              ...
                              <Select
                                ...
                                disabled={effectiveDisableFields}
                              >
                                <SelectTrigger className={`h-11 ${validationErrors.case_pan_id ? "border-red-500" : ""}`} disabled={effectiveDisableFields}>
                                  <SelectValue placeholder={t("categoryModal.selectCasePan", "Select Case Pan")} />
                                </SelectTrigger>
                                <SelectContent>
                                  {casePans.length > 0 ? (
                                    casePans.map((pan: CasePan) => (
                                      <SelectItem key={pan.id} value={pan.id.toString()}>
                                        {pan.name}
                                      </SelectItem>
                                    ))
                                  ) : (
                                    <SelectItem value={NO_CASE_PANS_VALUE} disabled>
                                      {t("categoryModal.noCasePans", "No case pans available")}
                                    </SelectItem>
                                  )}
                                </SelectContent>
                              </Select>
          ...
          <DialogFooter className="px-6 py-4 border-t">
            ...
            {isFormValid && !(isEdit && isSubCategoryEdit && isDetailLoading) && (
              <Button
